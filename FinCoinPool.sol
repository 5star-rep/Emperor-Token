// File: github/5star-rep/SOLIDITY-REP/Pool.sol
...
contract FINPOOL {
    ...
    constructor(address dead, IERC20 token) payable {
        Dead = dead;
        Contract = token;
    }
    ...
    function claim() public {
        uint256 burn = rewards / 2;
        uint256 finRewards = rewards /2;
        uint256 erc20balance = Contract.balanceOf(address(this));
        uint256 Liquidity = erc20balance - totalStaked;
        uint256 secondsStaked = block.timestamp - stakedFromTS[msg.sender];
        uint256 rewards = staked[msg.sender] * secondsStaked / 3.154e7;
        ...
        Contract.transfer(Dead, burn);
        Contract.transfer(msg.sender, finRewards);
        stakedFromTS[msg.sender] = block.timestamp;
    }
}
